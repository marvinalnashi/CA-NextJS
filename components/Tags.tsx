...
const TagsItem: React.FC<TagsItemProps> = ({ id, onClick, children }) => (
  <div key={id} className="tags-item" onClick={onClick}>
    {children}
  </div>
);
...
const Tags: React.FC = () => {
  ...
  useEffect(() => {
    const tiltNodes = Array.from(document.querySelectorAll(".tags-card")) as HTMLElement[];
  ...
  return (
    <div className="tags-container">
      <h1 className="text-align">Categories</h1>
      {categories.map((category) => (
        <div key={category}>
          <h2 className="text-align">{category}</h2>
          <div className="items-container">
            {items[category].map((item) => (
              <div key={item.id} className="tags-card-container">
                <div className="tags-card">
                  <div className="tags-content">
                    <TagsItem key={item.id} id={`item-${item.id}`} onClick={() => handleClick(item)}>
